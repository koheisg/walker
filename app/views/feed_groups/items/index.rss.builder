...
xml.rss(
  ...
  xml.channel do
    xml.title " #{@feed_group.name} (#{(@ref_time - 1.day).to_date})"
    # xml.description "#{@feed.name}の説明" # TODO
    xml.link feed_group_items_url(utm_medium: 'rss')
    ...
    @items_set.each do |date, items|
      xml.item do
        xml.title items.first(3).map(&:title).join(' / ').truncate(140)
        xml.link feed_group_daily_url(date: date.strftime('%Y%m%d'), utm_medium: 'rss')
        ...
        #   )
        # end
        xml.dc :creator, @feed_group.name
      end
    end
  end
end
