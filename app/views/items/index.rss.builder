...
xml.rss(
  ...
  xml.channel do
    xml.title " walker (#{(@ref_time - 1.day).to_date})"
    # xml.description "#{@feed.name}の説明" # TODO
    xml.link items_url(utm_medium: 'rss')
    ...
    @items_set.each do |date, items|
      xml.item do
        ...
        xml.link daily_url(date: date.strftime('%Y%m%d'), utm_medium: 'rss')
